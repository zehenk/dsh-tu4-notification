...
- **Configurable display duration**: Adjust `TOAST_SECONDS` constant in `index.js` (default: 10 seconds)
...
- **可配置的显示时长**：在 `index.js` 中修改 `TOAST_SECONDS` 常量（默认：10秒）
...
- **10 秒自动消失**，0.8 秒淡出；若被忽略，请求回退到 Web GUI 处理
- **DPI 感知**（200%+ 依然清晰）
...
dsh plugin --profile web add .\dsh-tu4-notification-0.5.1.tgz
...
approval/request
  → dsh-tu4-notification (prepend): show card with buttons
      ...
      └─ card ignored (≈13 s) or launch fails
...
# Pack
pnpm pack   # → dsh-tu4-notification-0.5.1.tgz
```
...
The plugin **claims** the approval while the card is up; if the card is ignored, the request is delegated to the Web GUI. The claim window is `TOAST_SECONDS + 3 s` (card display + a 3 s grace for launch + reaction), so by default ≈ 13 s. Override it with the `DSH_NOTIFY_CLAIM_MS` environment variable (milliseconds), e.g. in the DSH terminal before starting:

插件在卡片存续期间 **claim**（接管）审批；卡片被忽略时委托给 Web GUI。claim 窗口 = `TOAST_SECONDS + 3s`（卡片显示时长 + 3s 启动/反应余量），默认约 13 秒。可用环境变量 `DSH_NOTIFY_CLAIM_MS`（毫秒）覆盖，例如在 DSH 终端启动前设置：
...
- **No pending entry in the Web GUI right after the card appears** → by design: the card is claiming the approval for ≈13 s. Click a button to answer, or wait for the fade and the GUI entry appears.
...
- **卡片出现后 Web GUI 里暂时没有 pending 条目** → 设计如此：卡片正在 claim 审批（约 13 秒）。点按钮应答，或等淡出后 GUI 条目会出现。
